...
export default function PersonalizationConfigurator({ 
  ...
  const analyzeImage = (file: File, dataUrl: string): Promise<ImageValidation> => {
    return new Promise((resolve) => {
      ...
      if (file.type === 'image/svg+xml') {
        resolve({
          status: 'success',
          message: '✓ SVG-Grafik optimal für Gravur'
        })
        return
      }
      ...
      img.onload = () => {
        ...
        if (uniqueColors <= 8 && contrastRatio > 0.3) {
          resolve({
            status: 'success',
            message: '✓ Grafik geeignet für Gravur',
            details: hasTransparency ? 'Transparenz erkannt' : undefined
          })
        } else if (uniqueColors <= 20 || contrastRatio > 0.2) {
          resolve({
            status: 'warning',
            message: '⚠ Grafik möglicherweise komplex',
            details: 'Für beste Ergebnisse: Grafik mit klaren Konturen und wenig Details verwenden'
          })
        } else {
          resolve({
            status: 'warning',
            message: '⚠ Komplexe Grafik erkannt',
            details: 'Dieses Bild könnte für Lasergravur zu detailliert sein. Einfachere Grafiken mit klaren Konturen liefern bessere Ergebnisse.'
          })
        }
      }
  ...
  const handleFileSelect = useCallback(async (file: File, zone: "lid" | "bottom") => {
    ...
    if (file.size > 2 * 1024 * 1024) {
      setUploadError("Datei zu groß (max. 2MB)")
      return
    }
        ...
        <div className={`border-2 ${borderColor} rounded-lg p-3 ${bgColor}`}>
          <div className="flex items-center gap-3 mb-2">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-ui-fg-base">{t.logo} ✓</p>
              {validation && (
                <p className={`text-xs ${
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-ui-fg-muted whitespace-nowrap">
              {pricePerPersonalization.toFixed(2)} € {t.perOption}
            </span>
            <button
      ...
      {/* Maximize-Modus Header mit nur Titel und Schließen-Button */}
      {expanded && (
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <span className="text-lg">✨</span>
        ...
        <>
          ...
          <div className="mb-4">
            <div className="grid grid-cols-2 gap-2">
              ...
              >
                <span>{t.lid}</span>
                {(lidText || lidLogo) && <span className="text-black font-bold text-xs">✓</span>}
              </button>
              <button
              ...
              >
                <span>{t.bottom}</span>
                {(bottomText || bottomLogo) && <span className="text-black font-bold text-xs">✓</span>}
              </button>
            </div>
          </div>
            ...
            {!appliedCoupon ? (
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="px-4 py-2 text-sm bg-[#D4AF37] text-white rounded-lg hover:bg-[#B8944F] disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {t.redeem}
                </button>
              </div>
              ...
              <div className="flex items-center justify-between p-2 bg-green-50 border border-green-200 rounded-lg">
                <span className="text-sm text-green-700">
                  ✓ {appliedCoupon.code} ({appliedCoupon.discount}% {t.discount})
                </span>
                <button
            ...
            <div className="p-3 bg-ui-bg-base rounded-lg border border-ui-border-base">
              <div className="space-y-1 text-sm">
                <div className="flex justify-between text-ui-fg-muted">
                  ...
                  <span>{(personalizationCount * pricePerPersonalization).toFixed(2)} €</span>
                </div>
                {appliedCoupon && (
                  <div className="flex justify-between text-green-600">
                    <span>{t.discount} ({appliedCoupon.discount}%)</span>
                    <span>-{((personalizationCount * pricePerPersonalization) - totalPrice).toFixed(2)} €</span>
                  </div>
                )}
              ...
              <div className="mt-3 pt-3 border-t border-ui-border-base text-xs text-ui-fg-muted space-y-0.5">
                {lidText && <div>• {t.lidText}: "{lidText}" ({t.size}: {lidTextSettings.size}%)</div>}
                {lidLogo && <div>• {t.lidLogo} ✓ ({t.size}: {lidLogoSettings.size}%)</div>}
                {bottomText && <div>• {t.bottomText}: "{bottomText}" ({t.size}: {bottomTextSettings.size}%)</div>}
                {bottomLogo && <div>• {t.bottomLogo} ✓ ({t.size}: {bottomLogoSettings.size}%)</div>}
              </div>
            </div>
          )}
        </>
